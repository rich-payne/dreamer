test_that("MCMC: linear", {
  ...
  test_posterior(
    ...
    b1 = 1:10,
    b2 = 2:11,
    true_responses = rlang::expr(b1 + b2 * dose)
  )
  ...
  test_posterior(
    ...
    b1 = 1:10,
    b2 = 2:11,
    true_responses = rlang::expr(b1 + b2 * dose - (b1 + b2 * reference_dose))
  )
})

test_that("MCMC: linear long linear", {
  n_chains <- 2
  t_max <- 4
  times <- c(0, 2, 4)
  ...
  test_posterior(
    ...
    b2 = 2:11,
    a = 10:1,
    true_responses = rlang::expr(a + time / !!t_max * (b1 + b2 * dose))
  )
  test_posterior(
    mcmc,
    doses = c(1, 3, 2),
    ...
    true_responses = rlang::expr(
      a + (time / !!t_max) * (b1 + b2 * dose) -
        (a + time / !!t_max * (b1 + b2 * reference_dose))
    )
  )
})

test_that("MCMC: linear long ITP", {
  n_chains <- 2
  t_max <- 4
  times <- c(0, 2, 4)
  data <- dreamer_data_linear(
  ...
  test_posterior(
    ...
    true_responses = rlang::expr(
      a + (1 - exp(- c1 * time)) / (1 - exp(- c1 * !!t_max)) * (b1 + b2 * dose)
    )
  )
  test_posterior(
    mcmc,
...
test_that("MCMC: linear long IDP", {
  n_chains <- 2
  t_max <- 4
  times <- c(0, 2, 4)
  data <- dreamer_data_linear(
