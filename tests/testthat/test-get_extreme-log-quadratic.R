test_that("get_extreme.logquad()", {
  ...
  exp <- tibble::tibble(doses = c(5, 5, 2, 3)) %>%
    dplyr::mutate(
      extreme_responses = b1 + b2 * log(doses + 1) + b3 * log(doses + 1) ^ 2,
      greater = TRUE
    )
  ...
  exp <- tibble::tibble(doses = c(1.25, 1.5, 5, 1)) %>%
    dplyr::mutate(
      extreme_responses = b1 + b2 * log(doses + 1) + b3 * log(doses + 1) ^ 2,
      greater = FALSE
    )
...
test_that("get_extreme.logquad() longitudinal", {
  times <- c(0, 10)
  t_max <- max(times)
  ...
  exp <- tibble::tibble(doses = c(0, 0, 2, 3)) %>%
    dplyr::mutate(
      extreme_responses =
        a + (b1 + b2 * log(doses + 1) + b3 * log(doses + 1) ^ 2) * time / t_max,
      greater = TRUE
    )
  expect_equal(obs, exp)
