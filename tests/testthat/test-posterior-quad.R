test_that("MCMC: quadratic", {
  ...
  test_posterior(
    ...
    b2 = 2:11,
    b3 = 3:12,
    true_responses = rlang::expr(b1 + b2 * dose + b3 * dose ^ 2)
  )
...
test_that("MCMC: quadratic long linear", {
  n_chains <- 2
  t_max <- 4
  times <- c(0, 2, 4)
  ...
  test_posterior(
    ...
    a = 10:1,
    true_responses = rlang::expr(
      a + time / !!t_max * (b1 + b2 * dose + b3 * dose ^ 2)
    )
  )
  test_posterior(
    mcmc,
...
test_that("MCMC: quadratic long ITP", {
  n_chains <- 2
  t_max <- 4
  times <- c(0, 2, 4)
  data <- dreamer_data_linear(
...
test_that("MCMC: quadratic long IDP", {
  n_chains <- 2
  t_max <- 4
  times <- c(0, 2, 4)
  data <- dreamer_data_linear(
