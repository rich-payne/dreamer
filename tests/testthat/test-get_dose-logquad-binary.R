test_that("get_dose() log-quad binary logit", {
  link <- "logit"
  ...
  get_dose(
    mcmc$mod,
    time = NULL,
    response = ilogit(b1 + b2 * log(dose + 1) + b3 * log(dose + 1) ^ 2),
    lower = lower,
    upper = upper
  ) %>%
...
test_that("get_dose() log-quad binary probit", {
  link <- "probit"
  data <- dreamer_data_linear_binary(
    n_cohorts = c(10, 20, 30),
    dose = c(1, 3, 5),
  ...
  get_dose(
    mcmc$mod,
    time = NULL,
    response = iprobit(b1 + b2 * log(dose + 1) + b3 * log(dose + 1) ^ 2),
    lower = lower,
    upper = upper
  ) %>%
    expect_equal(rep(dose, 2))
...
test_that("get_dose() log-quad binary logit longitudinal", {
  link <- "logit"
  times <- c(0, 10)
  t_max <- max(times)
  data <- dreamer_data_linear_binary(
...
test_that("get_dose() log-quad binary probit longitudinal", {
  link <- "probit"
  times <- c(0, 10)
  t_max <- max(times)
  data <- dreamer_data_linear_binary(
